...
class Dense_decoder_small(nn.Module):
    def __init__(self):
        super(Dense_decoder_small, self).__init__()
        ############# Block5-up  16-16 ##############
...
class Dense(nn.Module):
    def __init__(self):
        ...
        self.decoder_A1 = Dense_decoder()
        self.decoder_A2 = Dense_decoder_small()
        self.decoder_T = Dense_decoder()
        self.decoder_B = Dense_decoder()
        self.convT1 = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1)
        self.convM = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1)
        self.ResT = ResidualBlock(32)
        self.ResM = ResidualBlock(32)
        self.convT = nn.Conv2d(32, 1, kernel_size=3, stride=1, padding=1)
        self.convM1 = nn.Conv2d(32, 2, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, opt):
        ...
        #x4 = self.residual_block43(x4)
        #x4 = self.residual_block44(x4)
        #x4 = self.residual_block45(x4)
        #x4 = self.residual_block46(x4)

        ######################################
        A1 = self.decoder_A1(x, x1, x2, x4, opt)
        A2 = self.decoder_A2(x, x1, x2, x4, opt)
        #R = self.decoder_R(x, x1, x2, x4, opt)
        #G = self.decoder_G(x, x1, x2, x4, opt)
        M = self.decoder_B(x, x1, x2, x4, opt)
        #A = torch.cat([R,G,B], 1)
        T = self.decoder_T(x, x1, x2, x4, opt)
        T = self.sigT(self.convT(self.ResT(self.convT1(T))))
        T = torch.cat([T,T,T], 1)
        M = self.sigM(self.convM1(self.ResM(self.convM(M))))
        invT = 1.0 - T
        ...
        dehaze2 = x - A2*invT
        dehaze2 = dehaze2/T 		
        dehaze = dehaze1*W + (1.0 - W)*dehaze2
        return dehaze, dehaze1, dehaze2, A1,A2,T,W,mask
#
#
